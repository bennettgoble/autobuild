...
from autobuild.autobuild_tool_install import open_package
...
class AutobuildTool(autobuild_base.AutobuildBase):
    ...
    def run(self, args):
        ...
        if not args.source_file:
            ...
            try:
                with open_package(args.source_file) as pkg:
                    metadata = pkg.metadata()
                    if not metadata:
                        raise FileNotFoundError(configfile.PACKAGE_METADATA_FILE)
